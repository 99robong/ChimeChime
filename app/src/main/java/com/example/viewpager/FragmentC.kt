...
class FragmentC : Fragment() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		prefs = mainActivity.getSharedPreferences("login_data", Context.MODE_PRIVATE)
	...
	override fun onCreateView(
		...
		var patient_pk = prefs.getString("RelationPatient", "")
		...
		timer(period = 10000) {
			prefs = mainActivity.getSharedPreferences("login_data", Context.MODE_PRIVATE)
			var patient_pk = prefs.getString("RelationPatient", "")
			if (!patient_pk.equals("")) {
				data.clear()
				val db = Firebase.firestore
				patient_id = patient_pk as String
				db.collection(patient_pk)
					.get()
